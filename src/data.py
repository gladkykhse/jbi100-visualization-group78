...
def compute_agg_lost_workday_rate(df, column=None):
    ...
    temp["lost_workday_rate"] = np.where(
        temp["total_hours_worked"] == 0, 0, temp["dafw_num_away"] / temp["total_hours_worked"] * 100_000
    )
...
def compute_agg_safety_score(df, column=None):
    ...
    stats["safety_score"] = (
        ...
        + 1.0 * stats["fatality_rate"]
        + 1.0 * stats["lost_workday_rate"]
        + 1.0 * stats["severity_index"]
        + 1.0 * stats["death_to_incident"]
    )
